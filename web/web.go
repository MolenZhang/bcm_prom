...
func writeCPUCfgToFile(ns, svc, receiver, threshold, duration string) string {
  ...
  IF sum(rate(container_cpu_usage_seconds_total { namespace =~ "%s.*",pod_name =~ "%s.*",container_name = "%s"}[5m])) > %s
  FOR %s
  LABELS {
	%s_receiver = "%s"
  }
  ANNOTATIONS {
	description = "服务 %s CPU使用个数超过 %s",
	summary = "服务 %s CPU 使用率较高"
  }
`, repNs, repSvc, ns, svc, svc, threshold, duration, svc, receiver, svc, threshold, svc)
...
func writeMemoryCfgToFile(ns, svc, receiver, threshold, duration string) string {
  ...
  ANNOTATIONS {
	description = "服务 %s 的内存使用超过 %s",
	summary = "服务 %s 的内存使用过高"
  }
`, repNs, repSvc, ns, svc, svc, intThr, duration, svc, receiver, svc, threshold, svc)
...
`, repNs, repSvc, ns, svc, svc, duration, repSvc, receiver, svc, ns, svc, duration, repNs, repSvc, ns, svc, svc, duration, repSvc, receiver, svc, ns, svc)
...
`, threshold, duration, k8sCluster, receiver, threshold)
